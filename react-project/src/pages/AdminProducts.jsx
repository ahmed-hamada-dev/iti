          ...
          <tbody key={debouncedSearch} className="divide-y divide-slate-100">
            {isLoading ? (
              ...
              products.map((product, index) => (
                <tr
                  key={product.id}
                  className={`hover:bg-slate-50/50 transition-colors animate-fade-in stagger-${(index % 5) + 1}`}
                >
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-1">
                      <Button variant="ghost" size="icon" onClick={() => handleOpenDialog(product)} className="text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg">
                        <Edit className="w-4 h-4" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => handleOpenConfirm(product.id)} className="text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg">
